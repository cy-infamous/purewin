...
var artifactDefinitions = []artifactDefinition{
	...
	{DirName: "out", Type: "out", Indicators: []string{}},
	{DirName: ".cache", Type: ".cache", Indicators: []string{}},
}
...
func scanDirectory(basePath, currentPath string, depth, maxDepth int, seenProjects map[string]bool, artifacts *[]ProjectArtifact) error {
	...
	for _, entry := range entries {
		...
		if strings.HasPrefix(name, ".") && name != ".next" && name != ".nuxt" && name != ".venv" && name != ".gradle" && name != ".idea" && name != ".cache" {
			continue
		}
	...
	for _, entry := range entries {
		...
		// Skip symlinks — avoid infinite recursion and out-of-scope deletion.
...
func SaveCustomScanPaths(configDir string, paths []string) error {
	...
	lines = append(lines, "# One path per line, environment variables like $HOME are supported")
	lines = append(lines, "")
