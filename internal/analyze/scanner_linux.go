...
// On Linux we check with os.Lstat instead of Windows reparse points.
...
func (s *Scanner) scanDir(entry *DirEntry) {
	...
	for _, e := range entries {
		...
		// Never follow symlinks — avoid infinite recursion and out-of-scope traversal.
		if isSymlink(childPath) {
			s.addWarning("skipping symlink: " + childPath)
			continue
		}
